...
export class FrameTable {
  ...
  constructor(section: Uint8Array | undefined) {
    if (!section) {
      return;
    }
    ...
    while (c.offset + 8 <= section.length) {
      ...
      if (id === 0xffffffff) {
        ...
        if (augmentation !== '') {
          // 'z' augmentations belong to .eh_frame; GCC's .debug_frame uses "" — skip anything else.
          c.offset = end;
          continue;
        }
  ...
  /** Registers of the caller of the frame executing at `pc` with `regs`. */
  unwind(
    pc: number,
    regs: ReadonlyArray<number | undefined>,
    readMem: (address: number) => number | undefined,
  ): UnwindResult | null {
    const fde = this.#find(pc);
    if (!fde) {
      return null;
    ...
    for (let r = 0; r < 16; r++) {
      ...
      if (!rule) {
        ...
        out[r] = r >= 4 && r <= 11 ? regs[r] : r === 13 ? undefined : r === 14 ? regs[14] : undefined;
        continue;
      }
      switch (rule.kind) {
        case 'undefined':
        ...
        case 'same':
          ...
          break;
        case 'offset':
          out[r] = readMem((cfa + rule.offset) >>> 0);
          break;
        case 'val_offset':
...
function runCfa(
  ...
  while (!c.eof) {
    ...
    if (high === 0xc0) {
      const r = initial?.rules.get(low);
      if (r) {
        state.rules.set(low, r);
      } else {
        state.rules.delete(low);
      }
      continue;
    }
    switch (op) {
